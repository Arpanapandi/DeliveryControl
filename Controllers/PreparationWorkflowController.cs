...
namespace DeliveryControl.Controllers
{
    public class PreparationWorkflowController : Controller
    {
        ...
        private async Task<List<string>> GetAllowedDockCodesAsync()
        {
            var userRole = User.FindFirst(System.Security.Claims.ClaimTypes.Role)?.Value;
            var userIdStr = User.FindFirst(System.Security.Claims.ClaimTypes.NameIdentifier)?.Value;
            
            if (userRole == "Admin") return new List<string>(); 
            
            if (int.TryParse(userIdStr, out int userId))
            {
                return await _context.UserDockAccesses
                    .Where(uda => uda.UserId == userId)
                    .Select(uda => uda.Dock.DockCode)
                    .ToListAsync();
            }
        ...
        public async Task<IActionResult> Index(DateTime? filterDate, string? vin)
        {
            ...
            // Terapkan filter dock jika bukan admin
            if (!isAdmin)
            {
                query = query.Where(s => allowedDockCodes.Contains(s.Area!) || (s.Customer != null && allowedDockCodes.Contains(s.Customer.Docking!)));
            }
        ...
        public async Task<IActionResult> LookupSchedule(string tag, string? label, string? kanban)
        {
            ...
            try
            {
                ...
                if (!isAdmin)
                {
                    scheduleQuery = scheduleQuery.Where(s => allowedDockCodes.Contains(s.Area!) || (s.Customer != null && allowedDockCodes.Contains(s.Customer.Docking!)));
                }
        ...
        if (!string.IsNullOrEmpty(kanban))
        {
            ...
            // Check if this is ADM customer
            bool isADM = (targetItem.Customer ?? "").ToUpper().Contains("ADM");

            if (isADM)
            {
                // LOGIKA KHUSUS ADM: Kanban scan contains Manifest/Schedule Number
                schedule = _context.DeliverySchedules
                    .Include(s => s.Customer)
                    ...
                    .AsEnumerable() // Evaluation in-memory for string contains
                    .FirstOrDefault(s => {
                        if (string.IsNullOrEmpty(s.ScheduleNumber)) return false;
                        // ScheduleNumber format in DB: "ManifestNum/HHMM-Seq" (e.g., ADM-25001/1430-1)
                        // Extract only the Manifest part (before '/') for matching
                        string manifestPart = s.ScheduleNumber.Split('/')[0].Trim().ToUpper();
                        return !string.IsNullOrEmpty(manifestPart) && (kanbanUpper.Contains(manifestPart) || manifestPart.Contains(kanbanUpper));
                    });

                // Fallback for ADM: If manifest matching fails, try standard FIFO matching (Safety net)
                if (schedule == null)
                {
                    bool matchPartNo = !string.IsNullOrEmpty(partNo) && (kanbanUpper == partNo || kanbanUpper.Contains(partNo) || VinHelper.Normalize(kanban) == VinHelper.Normalize(targetItem.CustomerPartNumber));
                    bool matchVin    = !string.IsNullOrEmpty(itemVin) && (kanbanUpper == itemVin || kanbanUpper.Contains(itemVin) || VinHelper.Normalize(kanban) == VinHelper.Normalize(targetItem.VIN));
                    bool matchCode   = !string.IsNullOrEmpty(targetItem.ItemCode) && (kanbanUpper == targetItem.ItemCode.ToUpper() || VinHelper.Normalize(kanban) == VinHelper.Normalize(targetItem.ItemCode));

                    if (matchPartNo || matchVin || matchCode)
                    {
                ...
                if (schedule == null)
                {
                    return Json(new { 
                        ...
                        message = $"KANBAN ADM TIDAK SESUAI! Pastikan scan Manifest/Schedule Number atau Part Number yang benar." 
                    });
                }
            }
            else
            {
                // LOGIKA UMUM: Kanban scan contains Part Number, VIN, or ItemCode (Support flexible LB prefix/suffix)
                bool matchPartNo = !string.IsNullOrEmpty(partNo) && (kanbanUpper == partNo || kanbanUpper.Contains(partNo) || VinHelper.Normalize(kanban) == VinHelper.Normalize(targetItem.CustomerPartNumber));
                bool matchVin    = !string.IsNullOrEmpty(itemVin) && (kanbanUpper == itemVin || kanbanUpper.Contains(itemVin) || VinHelper.Normalize(kanban) == VinHelper.Normalize(targetItem.VIN));
                bool matchCode   = !string.IsNullOrEmpty(targetItem.ItemCode) && (kanbanUpper == targetItem.ItemCode.ToUpper() || VinHelper.Normalize(kanban) == VinHelper.Normalize(targetItem.ItemCode));
                ...
                else
                {
                    return Json(new { 
                        ...
                        message = $"KANBAN tidak sesuai! Barcode harus mengandung kode: {(string.IsNullOrEmpty(partNo) ? (string.IsNullOrEmpty(targetItem.ItemCode) ? itemVin : targetItem.ItemCode) : partNo)}" 
                    });
                }
            }
        }
        ...
        public async Task<IActionResult> Save([FromBody] PreparationRecord record)
        {
            ...
            try 
            {
                ...
                // Check if this is ADM customer
                bool isADM = (item.Customer ?? "").ToUpper().Contains("ADM");
                DeliverySchedule? schedule = null;

                var today = DateTime.Today; // Ensure 'today' is defined here
                ...
                if (isADM)
                {
                    ...
                    schedule = _context.DeliverySchedules
                        ...
                        .FirstOrDefault(s => {
                            ...
                            // Extract only the Manifest part (before '/') for matching
                            string manifestPart = s.ScheduleNumber.Split('/')[0].Trim().ToUpper();
                            return !string.IsNullOrEmpty(manifestPart) && (kanbanSaveUpper.Contains(manifestPart) || manifestPart.Contains(kanbanSaveUpper));
                        });

                    // Fallback for ADM: If manifest matching fails, try standard FIFO matching (Safety net)
                    if (schedule == null)
                    {
                        bool matchPartNo = !string.IsNullOrEmpty(partNoSave) && (kanbanSaveUpper == partNoSave || kanbanSaveUpper.Contains(partNoSave) || VinHelper.Normalize(record.Kanban) == VinHelper.Normalize(item.CustomerPartNumber));
                        bool matchVin    = !string.IsNullOrEmpty(vinSave) && (kanbanSaveUpper == vinSave || kanbanSaveUpper.Contains(vinSave) || VinHelper.Normalize(record.Kanban) == VinHelper.Normalize(item.VIN));
                        bool matchCode   = !string.IsNullOrEmpty(item.ItemCode) && (kanbanSaveUpper == item.ItemCode.ToUpper() || VinHelper.Normalize(record.Kanban) == VinHelper.Normalize(item.ItemCode));
                    ...
                    if (schedule == null)
                    {
                        return Json(new { success = false, message = $"KANBAN ADM TIDAK SESUAI! Pastikan scan Manifest/Schedule Number atau Part Number yang benar." });
                    }
                }
                else
                {
                    // LOGIKA UMUM: Kanban scan contains Part Number or VIN (Support flexible LB prefix/suffix)
                    bool matchPartNo = !string.IsNullOrEmpty(partNoSave) && (kanbanSaveUpper == partNoSave || kanbanSaveUpper.Contains(partNoSave) || VinHelper.Normalize(record.Kanban) == VinHelper.Normalize(item.CustomerPartNumber));
                    bool matchVin    = !string.IsNullOrEmpty(vinSave) && (kanbanSaveUpper == vinSave || kanbanSaveUpper.Contains(vinSave) || VinHelper.Normalize(record.Kanban) == VinHelper.Normalize(item.VIN));
                    bool matchCode   = !string.IsNullOrEmpty(item.ItemCode) && (kanbanSaveUpper == item.ItemCode.ToUpper() || VinHelper.Normalize(record.Kanban) == VinHelper.Normalize(item.ItemCode));
                ...
                {
                    var allowedDockCodes = await GetAllowedDockCodesAsync();
                    bool isAuthorized = allowedDockCodes.Contains(schedule.Area!) || (schedule.Customer != null && allowedDockCodes.Contains(schedule.Customer.Docking!));
                    if (!isAuthorized)
                    {
